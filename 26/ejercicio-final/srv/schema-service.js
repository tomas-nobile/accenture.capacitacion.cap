...
module.exports = cds.service.impl(async (srv) => {
  srv.on("modifyQuantity", async (req) => {
    ...
    const selOrd = await cds.run(
        SELECT.from(Orders).where({ product_ID })
      );

      if(!product_ID || !quantity || !details_region) return 'Los campos son obligatorios'

      if(stockOrder) stock-=stockOrder;

      for (const e of selOrd) {
        stockOrder+=e.quantity
        stock -= e.quantity;
      }
    ...
    await cds.run(
      UPDATE(Products).set({ stock:stock-quantity, stockOrder:stockOrder+quantity }).where({ ID: product_ID })
    );
  });
});
